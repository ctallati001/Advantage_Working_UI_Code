			...
			dayMousedown: function(ev) {
				...
				var dragListener = new DragListener(this.coordMap, {
					...
					listenStop: function(ev) {
						...
						if((ev.target.nodeName == 'SPAN' || ev.target.nodeName == 'span') && ev.target.offsetParent.className == 'fc-week-number'){
							var date = ev.target.offsetParent.nextElementSibling.dataset.date;
							view.trigger('weekClick', ev.target.innerHTML, date, ev);
						}
						enableCursor();
					}
				});
			...
			renderMoreLink: function(cell, hiddenSegs) {
				...
				.on('click', function(ev) {
					...
					if (clickOption === 'popover') {
						reslicedAllSegs.sort(_this.compareMilli);
...
//		It is responsible for managing width/height.
		...
		var BasicView = fcViews.basic = View.extend({
			...
			render: function() {
				...
				this.weekNumberWidth = this.opt('weekNumbersWidth');
			...
			numberIntroHtml: function(row) {
				if (this.weekNumbersVisible) {
					...
					'<a><span>' + // needed for matchCellWidths
	...
	------------------------------------------------------------------------------------------------------------------*/
		...
		var YearView = fcViews.year = View.extend({
			...
			computeTitle: function() {
				if (this.opt('yearTitleFormat') !== null) {
					var title = this.intervalStart.year();
			...
			render: function(delta) {
				...
				this.intervalStart._locale = this.start._locale;
			...
			weekNumberClick: function(ev){
				$.trigger('weekNumberClick', ev);
			},
			// Create month grids
			buildDayGrids: function() {
				...
				$.each(nums, function(offset, m) {
					...
					view.dayGrid = dayGrid;
					view.coordMap = dayGrid.coordMap;
	...
	------------------------------------------------------------------------------------------------------------------*/
			...
			weekBind: function(days) {
				days.click(this.weekNumClick);
				//days.mousedown(this.daySelectionMousedown);
			},
			...
			weekNumClick: function(ev) {
//				if (!this.opt('selectable')) { // if selectable, SelectionManager will worry about dayClick
//				var match = this.find('span');
				var date = new Date();
				$.trigger('weekNumClick', this, date, true, ev);
//				}
			},
			// Renders the given events onto the view and populates the segments array
			renderEvents: function(events) {
